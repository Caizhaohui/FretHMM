...
def process_batch(
    ...
    if not files:
        ...
        return []
    return process_files(files, config, output_dir)
    ...
    try:
        if config.workers <= 1:
            for i, filepath in enumerate(files, 1):
                ...
                try:
                    result = process_trace_file(filepath, config, output_dir)
            ...
            with ProcessPoolExecutor(max_workers=workers) as pool:
                futures = {
                    pool.submit(process_trace_file, filepath, config, output_dir): index
