...
- **GUI 并行数**：GUI 中 Workers 默认值仅为 `2`，可选 `1`–`4`；CLI 的 `--workers` 默认值仍为 `1`。`1` 适合低内存设备，`2` 推荐用于普通笔记本，`3` 或 `4` 会占用更高的 CPU 和内存。并行仅发生在文件之间，每个文件作为一个任务处理。
...
- **后台线程处理**：所有分析任务在后台线程执行，支持随时取消（Cancel 按钮）。取消后不再调度新的文件；已在处理中的文件会完成，并保留其 `*_classified.csv` 输出。取消 Review Grid 时不会写入部分完成的审查图像。
...
Windows GUI 真·多进程并行，默认资源安全与取消语义加固：

- **GUI 并行数**：GUI Workers 默认 `2`，可选 `1`–`4`；CLI `--workers` 默认仍为 `1`。选择 `3` 或 `4` 时会提示更高 CPU/内存占用。并行仅发生在文件之间。
- **Windows 受限进程池**：分类与审查拼图任务通过 `spawn` 兼容进程池执行，使 Workers=2/4 在 Windows 上真正加速批量处理。
- **取消 / 关闭安全**：取消后不再调度新文件；进行中的文件可完成并保留 `*_classified.csv`。取消的审查拼图不发布半成品 PNG。若提交已成功再关窗，保持完成状态。
